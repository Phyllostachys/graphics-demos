...
Shader::Shader(std::string vert_filename, std::string geom_filename, std::string frag_filename)
{
    ...
    inFile = fopen(geom_filename.c_str(), "rb");
    if (inFile == NULL) {
        std::cout << "Geometry shader file doesn't exist.\n";
        anyErrors = true;
        return;
    }
    readFile(geomSource, inFile);
    ...
    typeFlags = VERT_SHADER | GEOM_SHADER | FRAG_SHADER;
    compiled = false;
    vertShaderHandle = 0;
    geomShaderHandle = 0;
    fragShaderHandle = 0;
...
bool Shader::compile()
{
    ...
    const GLchar* gs = geomSource.c_str();
    const GLint gsLen = geomSource.length();
    glShaderSource(geomShaderHandle, 1, &gs, &gsLen);
    glCompileShader(geomShaderHandle);

    // check geometry shader compile status
    if (!isShaderStatusGood(geomShaderHandle, GL_COMPILE_STATUS, buffer)) {
        anyErrors = true;
        return false;
    }
